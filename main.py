...
class GBStandardDownloader:
    ...
    def verify_code(self, code):
        ...
        if response.status_code == 200:
            if response.text == "success":
                ...
                print(f"验证码验证失败: {response.text}")
                return False
        else:
            print(f"提交验证码请求失败，状态码: {response.status_code}")
            return False
            
    def show_verify_code_dialog(self, verify_code_image, parent=None):
        """显示验证码输入对话框"""
        # 创建窗口
        if parent is None:
            # 如果没有父窗口，创建主窗口
        ...
        # 设置窗口图标
        try:
            root.iconbitmap("gb_downloader.ico")
        except Exception as e:
            print(f"设置验证码对话框图标失败: {e}")
        
        # 初始化变量
        code_var = tk.StringVar()
        result_var = tk.StringVar()
        result_var.set("请输入验证码")
        
        # 错误提示标签
        error_label = tk.Label(root, textvariable=result_var, fg="red")
        error_label.pack(pady=5)
        
        # 显示验证码图片
        image = Image.open(BytesIO(verify_code_image))
        photo = ImageTk.PhotoImage(image)
        image_label = tk.Label(root, image=photo)
        image_label.image = photo  # 保存引用，防止被GC回收
        ...
        code_entry = tk.Entry(input_frame, textvariable=code_var, font=("Arial", 12), width=20)
        code_entry.pack(side=tk.LEFT, padx=5)
        
        # 刷新按钮回调函数
        def refresh_code():
            # 获取新的验证码图片
            new_image_data = self.get_verify_code()
            if new_image_data:
                new_image = Image.open(BytesIO(new_image_data))
                new_photo = ImageTk.PhotoImage(new_image, master=root)
                image_label.config(image=new_photo)
                image_label.image = new_photo  # 更新引用
                result_var.set("请输入验证码")
                return new_image_data
            return verify_code_image
        
        # 刷新按钮
        refresh_btn = tk.Button(input_frame, text="刷新", command=refresh_code)
        ...
        # 确认按钮回调函数
        def on_confirm():
            nonlocal is_valid, entered_code
            entered_code = code_var.get().strip()
            if not entered_code:
                result_var.set("验证码不能为空")
                return
        ...
        # 确认按钮
        confirm_btn = tk.Button(btn_frame, text="确认", command=on_confirm, width=10)
    ...
    def generate_pdf(self, image_files, standard_number):
        ...
        # 生成包含日期时间戳的文件名
        output_pdf = f"{safe_standard_number}_{current_time}.pdf"
        ...
        try:
            ...
            for filename in os.listdir(self.output_dir):
                ...
                if os.path.isfile(file_path):
                    # 删除文件
                    os.remove(file_path)
                    print(f"已删除临时图片: {filename}")
            print(f"已清空图片文件夹: {self.output_dir}")
        except Exception as e:
            print(f"清空图片文件夹失败: {e}")
    ...
    def run(self, parent=None):
        ...
        for attempt in range(max_attempts):
            verify_code_image = self.get_verify_code()
            if verify_code_image:
                is_valid, entered_code = self.show_verify_code_dialog(verify_code_image, parent)
                if is_valid:
                    break
                print(f"验证码验证失败，尝试次数: {attempt+1}/{max_attempts}")
            else:
                print(f"获取验证码图片失败，尝试次数: {attempt+1}/{max_attempts}")
